...
void setup() {
  ...
  Firebase.RTDB.setInt(&fbdo, "PATIENT01", patient2);
  Firebase.RTDB.setInt(&fbdo, "DOCTOR01", buttonState2);
...
static void sendOne() {
  if (Firebase.RTDB.setInt(&fbdo, "DOCTOR01", buttonState2)) {
    lcd.clear();
    lcd.setCursor(0, 0);
...
static void sendTwo() {
  if (Firebase.RTDB.setInt(&fbdo, "PATIENT01", patient2)) {
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("Passed the value");
    delay(100);
...
static void getValue() {
  if (Firebase.RTDB.getInt(&fbdo, "/ROOMS/ROOM01")) {
